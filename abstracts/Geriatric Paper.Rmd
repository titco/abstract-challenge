...
dtt <- n  %>% dplyr::select(pid, age:ti, sbp_1, gcs_t_1, tos, losu, ct, fast, xray, dama, dodd, todd, iss, niss, died, allinj, tbi3)

sapply(dtt, function(x) sum(is.na(x)))

#Complete case analysis
dtt$complete.cases <- complete.index <- complete.cases(dtt)

n.incomplete <- sum(!complete.index)
p.incomplete <- round((n.incomplete/nrow(dtt)) * 100,1)
dt <- dtt[complete.index, ]

#----------------- Missing values comparison table -----------------------

myvars <- c("age","sex", "tran", "moi", "mot", "ti", "sbp_1", "gcs_t_1" , "losu", "ct", "dama",  "iss", "niss", "tbi3","complete.cases")
catvars <- c("sex", "tran", "moi","mot", "ti", "ct", "dama","tbi3")

Table0 <- CreateTableOne(vars = myvars, data=dtt ,strata = "complete.cases", factorVars = catvars)
knitr::kable(print(Table0, nonnormal = TRUE, showAllLevels = TRUE, printToggle = FALSE, varLabels = TRUE, formatOptions = list(big.mark = ",")))

variable_median <- c("sbp_1", "gcs_t_1",  "iss")
...
m2 <- glm(died~agegrp+sex+moi_collapsed+mot_collapsed+sbp_cat+gcs_cat+tbi3+iss_cat, data = dt,  family = binomial(link = "logit"))
summ(m2, exp = T)
...
dt$gcs_cat <- relevel(dt$gcs_cat, ref = "Mild")

u7 <- glm(died~gcs_cat, data = dt,  family = binomial(link = "logit"))
summ(u7, exp = T)
tbl_regression(u7, exponentiate = TRUE)

u8 <- glm(died~age, data = dt,  family = binomial(link = "logit"))
summ(u8, exp = T)
tbl_regression(u8, exponentiate = TRUE)

table(dt$sex, dt$moi_collapsed)
