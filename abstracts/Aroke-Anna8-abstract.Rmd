...
In the TTRIS dataset, `r dm` patients were triaged green. The mean age of these patients was `r mean_age` (+-`r stddev_age`). `r percent_agebelow65`% of the patients were below 65 years of age. `r percent_male`% patients were male, while `r percent_female`% patients were female. `r percent_blunt`% patients had blunt injuries. Most common mode of arrival to the ED was through `r mot1` (`r percent_mot_pvt`)%. `r percent_mot_onfoot`% patients came to the ED on foot. `r moi1` were the most common cause of injury (`r p_assault`%) followed by `r moi2` (`r p_tra`%) and `r moi3` (`r p_fall`%). Injury due to animal bites were `r p_animalbite`. `r p_followup`% patients were successfully followed up at 6months, of which `r n_outcome_dead` patients were reported dead.
...
## Setting
This study took place in the Emergency Department at the K.B. Bhabha Hospital, a regional secondary healthcare facility located in the city of Mumbai.The ED serves approximately 350 patients each day, having medicinal, surgical or traumatic emergency. Being located near a police station the ED also serves a stop for health-checkup's of prisoners or arrested personnels.The ED has 5 bed's and is staffed by one physician/Casualty Medical Officer (CMO) two On-duty nurses, two dressers and two ward boys. 

## Data extraction
Data of patients triaged green were extracted from the Trauma Triage Study in India (TTRIS) dataset. This dataset contains patients presenting to the ED with any of the mechanism of Injury listed in International Classification of Diseases (ICD-10) between V01 to Y09. Patients aged 18 and above were included in the dataset. Data was collected prospectively from the year 2016 to 2019. Follow-up was done at 24hrs, 30 days and 6months from the day of injury. A retrospective cohort analysis of the data was performed using R. 

## Data Manipulation

- Mechanism of Injury: Mechanism of injury codes were grouped into Transport accidents (V00-V99), Falls (W00-W19), Animal Bites (W50-W64), Assault (X85-X99, Y00-Y09) and others. 

Severity of Injury was assesed using ISS and RTS scores[@champion1989]

- ISS: The Injury Severity Score (ISS) was categorized 'No defined injuries' (ISS=0 ), 'minor' (ISS<9), 'moderate' (ISS<15), 'severe' (ISS<25), 'profound' (ISS>=25).
...
Table1 shows Characteristics of the patient injury were most commonly `r inj1` (`r p_supf`%), followed by `r inj2` (`r p_ndi`%), `r inj3` (`r p_ow`%) and `r inj4` (`r p_frac`%). The Injury Severity Score (ISS) indicated that 50.2% were 'minor', 49.4% patients had 'No defined injuries', the reamining 0.4% patients had 'moderate' severity. Patients with ISS severe than 'moderate' were not present in those triaged green.

Table2 shows, as per the iss scores there were no patients with severe injuries. Among patients with animal bites, 67.36% had no defined injuries while 32.64% had minor. 

Table3 shows, The mechanism of injury of patients with moderate injuries were Fall, Assualt and others. 90.5% patients with animal bites had superficial injuries, while 6.75% had no defined injuries and 2.34% patients had open wound. In patients with assualt, 42.18% has superficial injuries followed by no defined injuries in 30.68% patients and 24.48% patients having open wound injuries. 
...
knitr::kable(print(Table1, caption = "Table 1. Demographic, Physiological parameters, Injury Characteristics", showAllLevels = TRUE, printToggle = FALSE, varLabels = TRUE))



Table2 <- colPerc(xtabs(~kbbh$isspv+kbbh$moi.collapsed, data = kbbh))
kable(Table2, 
      caption = "Table 2. Percentage distribution of Injury severity among different Mechanism of Injury",
      )
...
kable(Table3, 
      caption = "Table 3. Percentage distribution of Injury type among different Mechanism of Injury",
)
...
corrplot(chisq$residuals, is.cor = FALSE, method = "ellipse")
...
corrplot(d$residuals, is.cor = FALSE, method = "ellipse")
