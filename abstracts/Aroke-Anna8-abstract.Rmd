...
# Age
mean_age <- round(mean(kbbh$age),1)
stddev_age <- round(sd(kbbh$age),1)
...
Data of patients triaged green were extracted from the Trauma Triage Study in India dataset. A retrospective cohort analysis of the data was performed. 

## Results

In the TTRIS dataset, `r dm` patients were triaged green. The mean age of these patients was `r mean_age` (+-`r stddev_age`). `r percent_agebelow65`% of the patients were below 65 years of age. `r percent_male`% patients were male. `r percent_blunt`% patients had blunt injuries. Most common mode of arrival to the ED was through `r mot1` (`r percent_mot_pvt`)%. `r percent_mot_onfoot`% patients came to the ED on foot. `r moi1` was the commonest cause of injury (`r p_assault`%) followed by `r moi2` (`r p_tra`%) and `r moi3` (`r p_fall`%). Injury due to animal bites were `r p_animalbite`%. `r p_followup`% patients were successfully followed up at 6months, of which `r n_outcome_dead` patients were reported dead.

## Conclusion

Among the patients triaged green, injury due to assualt and transport accidents were predominant. All patients had an RTS score of greater than 4. Most of the patients had minor injuries or no defined injuries.
...
In the ED, patients are sorted for treatment by priority using triage.
Based on injury severity and physiological parameters patients are triaged as red, orange, yellow, or green, from most to least urgent. There are different triage systems applied in EDs across the world.
Most often EDs are perceived to provide quick and easy access to healthcare, although they are not. This leads to visits of patients with no injury or minor injuries.[@Uscher-Pines2013][@Coster2017]. These are non-urgent visits or ones that do not require an ED visit. 
Although these patients are triaged green they consume significant resources in the form of time, man-power. This also leads to poor patient outcome and has effect on waiting times of patients having more urgent needs.[@morley2018]

Studies have categorised patients triaged (according to the MTS Triaging system) green and blue as not very urgent and non-urgent respecticely.[@moura2020][@slagman2019]
...
In India, no objective data is available to understand the problem of overcrowding.
...
## Setting
We performed this study in the Emergency Department at the K.B. Bhabha Hospital, a regional secondary healthcare facility located in Mumbai, India. The ED serves approximately 350 patients each day, having medicinal, surgical or traumatic emergency. The close proximity to a police station entails this ED as a stop for health-checkups of prisoners and offenders.The ED has 5 bed's and is staffed by one physician/Casualty Medical Officer (CMO) two On-duty nurses, two dressers and two ward boys. 

## Data extraction
Data of patients triaged green were extracted from the Trauma Triage Study in India (TTRIS) dataset. Triage category was determined by the treating physician. Green was assigned to the least serious patients and those that should be treated last. Number of serious injuries was estimated by the treating pyhsician. A serious injury here was deinfed as injury that warrants hospitalization. This dataset contains patients presenting to the ED with any of the mechanism of Injury listed in International Classification of Diseases (ICD-10) between V01 to Y09. Patients aged 18 and above were included in the dataset. Data was collected prospectively from the year 2016 to 2019. Consent for telephonic follow-up was taken before the patient was discharged. Follow-up was done at 24hrs, 30 days and 6months from the day of injury. Data analysis was performed using R statistical software.

## Data Manipulation

- Mechanism of Injury: Mechanism of injury (ICD-10) codes were grouped into Transport accidents (V00-V99), Falls (W00-W19), Animal Bites (W50-W64), Assault (X85-X99, Y00-Y09) and others. 
...
- ISS: The Injury Severity Score (ISS) was categorized 'No defined injuries' (0), 'mild' (3-8), 'moderate' (9-15), 'severe' (16-25), 'profound' (>25).
...
There was `r nrow(dt)` patients in the study data. Out of these, `r n.incomplete` (`r p.incomplete` %) observations were incomplete and were excluded from the study population.

In the TTRIS dataset, `r dm` patients were triaged green. The mean age of these patients was `r mean_age` (+-`r stddev_age`). `r percent_agebelow65`% of the patients were below 65 years of age which mainly constitues the working population. Economically productive age group of 25-44 were the majority (49.4%), followed by patients between the age group of 18-24 (31.7%). `r percent_male`% patients were male. `r percent_blunt`% patients had blunt injuries. Most common mode of arrival to the ED was through private vehicles (`r percent_mot_pvt`)%. `r percent_mot_onfoot`% patients came to the ED on foot. `r moi1` was the commonest cause of injury (`r p_assault`%) followed by `r moi2` (`r p_tra`%) and `r moi3` (`r p_fall`%). Injury due to animal bites were `r p_animalbite`%. Of those transfered from other hospitals or local clinics (134), 61 had superficial injuries (40) or no defined injuries (21), 36 had fracture, 27 had open wound.

The Injury Severity Score (ISS) indicated that 50.2% were 'mild', 49.4% patients had 'No defined injuries', the remaining 0.4% patients had 'moderate' severity. `r p_followup`% patients were successfully followed up at 6months, of which `r n_outcome_dead` patients were reported dead. All patients had an RTS score greater than 4. `r p_rts_8`% patients had an RTS score of 8.

Patients with amputations had a single finger (8), thumb (2) or single toe amputations (2). Crush injuries were of unspecified part of the hand (3) the finger (1) and the toe (2). More than one third of the fractures were forearm and elbow fractures (73), followed by hand (53), foot fracture (50) and knee or lower leg fracture (44). Shoulder or upperarm fracture were 28, hip and thigh fractures were 19 and fracture of skull or facial bones were 19.
...
var_label(kbbh) <- list(age = "Age", agegrp = "Age Group", sex = "Sex", moi.collapsed = "Mode of Injury", tyi = "Type of injury", mot = "Mode of transport", tran = "Transfer status", avpu = "AVPU", sbp = "Systolic blood pressure", dbp = "Distolic blood pressure", hr = "Heart rate", spo2 = "Oxygen saturation", vo2 = "Need for oxygen support", rr = "Respiratory rate", nsi = "Number of serious injuries", hd = "Hospital disposition", gcs = "GCS", gcs.cat = "GCS",isspv = "ISS", rts = "RTS", inj_type = "Characteristic of Injury")

myvars <- c("age", "agegrp", "sex", "moi.collapsed", "tyi", "mot",
"tran", "avpu", "sbp", "dbp", "hr", "spo2", "vo2", "rr", "nsi", "hd", "gcs","gcs.cat", "isspv", "rts","inj_type")

catvars <- c("sex", "moi.collapsed", "tyi", "mot", "tran", "avpu", "vo2", "nsi", "hd","gcs.cat", "inj_type")

Table1 <- CreateTableOne(vars = myvars, data=kbbh, factorVars = catvars)
...
Table5 <- rowPerc(xtabs(~kbbh$inj_type+kbbh$agegrp, data = kbbh))
kable(Table5,
