...
The Injury Severity Score (ISS) indicated that 50.2% were 'mild', 49.4% patients had 'No defined injuries', the reamining 0.4% patients had 'moderate' severity. `r p_followup`% patients were successfully followed up at 6months, of which `r n_outcome_dead` patients were reported dead. `r p_rts_8`% patients had an RTS score of 8.
...
catvars <- c("sex", "moi.collapsed", "tyi", "mot", "tran", "avpu", "vo2", "nsi", "hd", "inj_type")
...
```
```{r}
table(kbbh$inj_type,kbbh$ardd)

Table3 <- xtabs(~kbbh$inj_type+kbbh$ardd, data = kbbh)
kable(Table3,
      caption = "Percentage distribution of Injury type among different Mechanism of Injury",
)
```
...
A chi-square test was performed to investigate the association between type of injury and age. A strong association with a p-value of (1.486e-06) was identified.
...
kbbh$inj_arr <- if_else(injar.time<2,"0-1",">1")

# data:  injar.time and kbbh$inj_type
# X-squared = 265.86, df = 220, p-value = 0.01868

end.time <- paste(kbbh$dodd , kbbh$todd)
start.time <- paste(kbbh$doar, kbbh$toar)
ardd.time <- round(difftime(end.time,start.time, units = "hours"))
kbbh$ardd <- if_else(ardd.time<2,"0-1",">1")
