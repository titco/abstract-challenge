...
# creating tbi dataset with 24h mortality

s24hrs <- tbi %>% filter(s24h == "1")

s24h <- s24hrs$s24h <- replace(s24h,s24h == 999 , NA)
...
## calculation for different trauma score for 30 days mortality using dataset s30days
...
gcss24 <- s24hrs["gcss24"] <- replace(gcss24,gcss24 > 15 ,NA)
