...
# finding time of death or discharge from date of admission


col4 <- c("doa","toa")
s<- tbi$date_time_admission <- apply (tbi[,col4],1, paste,collapse ="")
dtoa<- as.POSIXct(s)


col5 <- c("dodd","todd")
t <- tbi$date_time_deathordischarge <- apply (tbi[,col5],1,paste,collapse ="")
dtodd <- as.POSIXct(t)

delay <-difftime(date_time_deathordischarge, date_time_admission)

tbi["delay"] <- delay_hours <- as.numeric(delay/60)


## i want to create a table to compare characterics of dead and alive patients(30 days mortality characteristics)

hd1 <- tbi$hd1

tbi$hd1 <- ifelse(hd == 0,"discharge",
                 ifelse(hd == 1,"death",NA)
)
...
table4 <- CreateTableOne(vars = myvars, strata = "hd1" ,data=tbi, factorVars = catvars)

table4 <- knitr::kable(print(table4, caption = "Table 1. Demography of TBI Patients", showAllLevels = TRUE, printToggle = FALSE))


## calculation for different score
...
sbpmgap <- tbi$sbp
sbpmgap [sbp >= 120] <- 5
sbpmgap [sbp >= 60 & sbp <120] <- 3
sbpmgap [sbp < 60] <- 0


## calculation for mgap

tbi["mgap"] <- mgap <- as.numeric(tyimgap) + as.numeric(gcs) + as.numeric(agemgap) + as.numeric(sbpmgap)
...
gcsrts <- gcs
gcsrts[gcs >= 13] <- 4
gcsrts[gcs >= 9 & gcs < 13] <- 3
gcsrts[gcs >= 6 & gcs < 9] <- 2
gcsrts[gcs >= 4 & gcs < 6] <- 1
gcsrts[gcs < 4] <- 0
...
sbprts <- sbp
sbprts[sbp > 89] <- 4
sbprts[sbp >=76 & sbp <= 89] <- 3
sbprts[sbp >= 50 & sbp <= 75] <- 2
sbprts[sbp >= 1 & sbp <= 49] <- 1
sbprts[sbp == 0] <- 0
## Formula for calculating RTS

gcsrts <- tbi$gcsrts <- 0.9368*as.numeric(gcsrts)

sbprts <- tbi$sbprts <- 0.7326*as.numeric(sbprts)

rrrts <- tbi$rrrts <- 0.2908*as.numeric(rrrts)

tbi["rts"] <- rts <- tbi$rts <- round((gcsrts + sbprts + rrrts),1)
